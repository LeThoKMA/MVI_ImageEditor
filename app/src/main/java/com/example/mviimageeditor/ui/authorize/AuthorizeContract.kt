...
interface AuthorizeContract : ContractViewModel<AuthorizeContract.State, AuthorizeContract.Event, AuthorizeContract.Effect> {
    data class State(
        val authorizeResponse: AuthorizeResponse? = null,
    )

    sealed class Event {
        data class OnAuthorize(
    ...
    sealed class Effect {
        ...
        data object AuthorizeSuccess : Effect()
    }
}
